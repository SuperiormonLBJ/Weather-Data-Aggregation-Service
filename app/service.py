...
from app.http_client import get_global_session
...
class WeatherAggregationService:
    ...
    def __init__(self):
        # Session managed globally with connection pooling
    ...
    async def _fetch_all_providers(self, session: aiohttp.ClientSession, location: str, is_coords: bool, 
        ...
        tasks = [
            self._fetch_openweather(session, location, is_coords, openweather_key),
            self._fetch_weatherapi(session, location, weatherapi_key),
            self._fetch_openmeteo_with_location(session, location, is_coords, openweather_key)
        ]
    ...
    async def _fetch_openweather(self, session: aiohttp.ClientSession, location: str, is_coords: bool, api_key: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = await make_api_request(session, url, params, "openweather", "OpenWeatherMap")
    ...
    async def _fetch_weatherapi(self, session: aiohttp.ClientSession, location: str, api_key: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = await make_api_request(session, url, params, "weatherapi", "WeatherAPI")
    ...
    async def _fetch_openmeteo(self, session: aiohttp.ClientSession, lat: float, lon: float) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = await make_api_request(session, url, params, "openmeteo", "OpenMeteo")
    ...
    async def _fetch_openmeteo_with_location(self, session: aiohttp.ClientSession, location: str, is_coords: bool, 
        ...
        try:
            if is_coords:
                lat, lon = parse_coordinates(location)
                return await self._fetch_openmeteo(session, lat, lon)
            else:
                # Geocode first
                coords = await self._geocode_location(session, location, openweather_key)
                if coords:
                    lat, lon = coords
                    return await self._fetch_openmeteo(session, lat, lon)
        except Exception as e:
    ...
    async def _geocode_location(self, session: aiohttp.ClientSession, city_name: str, api_key: str) -> Optional[tuple]:
